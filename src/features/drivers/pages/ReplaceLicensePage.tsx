...
import { ReplacementPreviewCard } from "../components/ReplacementPreviewCard";

export function ReplaceLicensePage() {
  ...
  const errorMessage =
    ...
    t("drivers.replaceLicense.licenseNotFound");
    ...
    <div className="space-y-6 p-6">
      ...
      <div>
        <h1 className="text-2xl font-bold">
          {t("drivers.replaceLicense.pageTitle")}
        </h1>
        <p className="text-sm text-muted-foreground mt-1">
          {t("drivers.replaceLicense.pageDescription")}
        </p>
      </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">
            {t("drivers.replaceLicense.searchTitle")}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex gap-2">
            <Input
              type="number"
              min={1}
              placeholder={t("drivers.replaceLicense.licenseIdPlaceholder")}
              value={inputValue}
            ...
            <Button onClick={handleFind} disabled={isFetching || !inputValue}>
              ...
              <span className="ml-2">
                {t("drivers.replaceLicense.findBtn")}
              </span>
            </Button>
          </div>
        </CardContent>
      ...
      {(license || isFetching) && (
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">
              {t("drivers.replaceLicense.currentLicenseTitle")}
            </CardTitle>
          </CardHeader>
          <CardContent>
      ...
      {license && searchedId && (
        <ReplacementPreviewCard licenseId={searchedId} />
      )}
    </div>
  );
}

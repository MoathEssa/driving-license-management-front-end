...
/** True if `pathname` matches this item or any descendant. */
...
/**
 * Uses ChevronDown rotated via a stable className ternary — no dynamic
 * Tailwind group-data-[...] names, so JIT always generates the classes.
 *
 * LTR open=false  → -rotate-90  (points →)
 * RTL open=false  →  rotate-90  (points ←)
 * any open=true   →  rotate-0   (points ↓)
 */
...
// ─── Level-2+ sub-item (lives inside a SidebarMenuSub) ───────────────────────
...
function SubNavItem({ item }: { item: NavItem }) {
  ...
  // Controlled open — avoids dynamic Tailwind group-selector names
...
export function SidebarNav() {
  const { t } = useTranslation();
  const userRoles = useAppSelector((s) => s.auth.authData?.user.roles ?? []);
